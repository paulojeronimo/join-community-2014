...
* Digite <Ctrl b %>, na painel do tmux que está executando o rakel. Isso abrirá um novo painel a sua direita. Em seguida, execute:
[source,bash]
----
...
* Volte para a janela que está executando o awestruct via rake (<Ctrl b o>). Dê um <Ctrl c> no servidor. Em seguida, execute:
[source,bash]
----
$ tree _site/ | less
----
